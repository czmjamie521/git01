                                ...
                                <div class="media">
                                    ...
                                    <div class="media-body"> 
                                        ...
                                        <span class="date">${myDate}</span>
                                     	<input type="hidden" id="name-${item.commentid}" value="${item.uname}"/>
                                        <p>${item.content}</p>
		...
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion" 
				   href="#collapseOne${item.commentid}">
					展开评论
				</a>
			</h4>
		</div>
		<div id="collapseOne${item.commentid}" class="panel-collapse collapse in">
			<div class="panel-body" id="div_${item.commentid}">
			
				
				
			   <c:forEach items="${courseComment}" var="item1">
                     <c:if test="${item1.parentcomment==item.commentid }">
                      
                                <div class="media">
                                    <img class="mr-3" src="assets/img/tutor/${item1.head}" alt="">
                                    <div class="media-body"> 
                                        <h5 class="mt-0">${item1.uname} </h5>
                                        <fmt:formatDate value="${item1.publishtime }" pattern="yyyy-MM-dd hh:mm" var="myDate"/>
                                        <span class="date">${myDate}</span>
                                       <c:forEach items="${courseComment}" var="item2">
                                       <c:if test="${item1.reply==item2.commentid }">      
                                      <h6>回复(${item2.uname})</h6>
                                      </c:if>
                                      </c:forEach>
                                        <p>${item1.content}</p>
                                 <input type="hidden" id="name${item1.commentid}" value="${item1.uname}" />
                                 <a href='javascript:void(0)' onclick="reply('${item1.commentid}','${item.commentid}',${item.courseid})">评论</a>
                                 <span id="replywarn${item1.commentid}" ></span>
                                 ...
                                 <a href='javascript:void(0)' onclick="reply('${item.commentid}','${item.commentid}',${item.courseid})">评论</a>
                            ...
                            <div class="comment-form-area mt-30" >
                                ...
                                <div class="comment-form">
                                        ...
                                        <div class="text-center">
                                            <button  onclick="commentSubmit('${item.commentid }',${singleCourse.courseid})" class="btn default-btn">提交</button><span id="warn" ></span>
...
<br><br><br><br>	
                ...
                <c:forEach items="${page.dataList }" var="item">
                            ...
                            <div class="col-sm-6 col-lg-6  col-xl-4"  Style="float:left">
	        ...
	        <div class="modal-content">
	            ...
	            <div class="modal-body">
	            	 <input type="hidden" id="CommentId" value="" />
	            	 	 <input type="hidden" id="CommentId1" value="" />
