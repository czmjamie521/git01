...
public class CartDao {
	...
	/**
	 * 用户批量删除购物车条目使用, 拼接sql的where子句  cartitemid in (?,?,...)
	 * @param len
	 * @return
	 */
	...
	public List<CartItem> showCart(Integer uid) throws SQLException {
		String sql ="select cart.cartitemid, cart.quantity, course.* "+ 
	...
	//根据给定的主键值, 寻找CartItem对象(含完整的course)
	public CartItem findCartItem(String cartItemId) throws SQLException {
		String sql ="select cartitemid, cart.quantity, course.* "+ 
				"from t_cartitem cart LEFT JOIN t_course course on cart.courseid =course.courseid "+
				"where cartitemid=?";	
	 ...
	 */
	public void cartdelete(String cartItemIds) throws SQLException, CartDeleteException {
		//前台传来的cartItemIds 转化为数组
		Object[] cartItemArray = cartItemIds.split(",");
		String whereSql =toWhereSql(cartItemArray.length);
		//拼接出完整的sql语句
		String sql ="delete from t_cartitem where "+whereSql;
		
		//注意, 可变参数的类型必须是 Object[] 或者Object...
		if(qr.update(conn, sql, cartItemArray) ==0){
			throw new CartDeleteException("请勿重复删除!");
		}
	}
